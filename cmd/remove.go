...
func (r *RemoveCommand) Run() error {
    ...
    if len(removed) == 0 {
        fmt.Fprintln(os.Stderr, ui.Error("No specified keys found in project configuration"))
        fmt.Fprintln(os.Stderr, ui.Dim("Use 'bnv list' to see configured keys"))
        os.Exit(1)
    }
    ...
    // 5. Print results
    if len(removed) == 1 {
        fmt.Println(ui.Success(fmt.Sprintf("Removed %s from project", removed[0])))
    } else {
        fmt.Println(ui.Success(fmt.Sprintf("Removed %d keys from project", len(removed))))
        fmt.Println(ui.BulletList(removed))
    }
    ...
    fmt.Println(ui.Dim("Note: The keys still exist in the global store. Use 'bnv delete' to remove them completely."))
