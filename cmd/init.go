...
var initCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        ...
        fmt.Println(ui.Success("Created .better-env configuration file."))
        fmt.Println(ui.KeyValue("Linked store", storePath))
        fmt.Println()
        fmt.Println(ui.Section("Next steps"))
        fmt.Println(ui.NumberedList([]string{
            "Add secrets to your store: bnv add KEY",
            "Load secrets in this project: bnv load",
        }))
        return nil
        },
