...
func (r *RenameCommand) Run() error {
    ...
    fmt.Println(ui.Success(fmt.Sprintf("Renamed %s to %s", r.oldKey, r.newKey)))
    fmt.Println(ui.Warning("Project .better-env files may still reference the old key name. Update them manually."))

    // 11. Offer to update current project's .better-env if it exists
    if err := r.updateCurrentProject(); err != nil {
        fmt.Fprintln(os.Stderr, ui.Warning(err.Error()))
    }
...
func (r *RenameCommand) updateCurrentProject() error {
    ...
    fmt.Printf("\n%s %s ", ui.Prompt(".better-env file detected in current directory. Update it?"), ui.Dim("(y/n)"))
    ...
    fmt.Println(ui.Success("Updated current project's .better-env file"))
    return nil
}
